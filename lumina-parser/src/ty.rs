...
impl<'a> Parser<'a> {
    ...
    pub fn type_without_params(&mut self) -> Option<Tr<Type<'a>>> {
        self.r#type(false, false)
    }
        ...
        select! { self, "a type", span sensitive: newline_sensitive;
            ...
            T::Operator => {
                if self.take(span).bytes().all(|b| b == b'*') {
                    ...
                    self.err_expected_but_got(span, "type", "operator");
                    None
                }
            }
        }
        ...
        CON: FnOnce(Vec<Tr<Type<'a>>>, Box<Tr<Type<'a>>>) -> OUT,
    {
        ...
        loop {
            select! { self, "`->`, `)` or a type in function type", span peeked: true;
                T::Arrow => {
                  self.progress();
