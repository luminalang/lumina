...
use crate::prelude::*;
use crate::{debuginfo::Item, ProjectInfo, Target};
use ast::attr::Repr;
use derive_new::new;
use either::Either;
...
impl Item {
    fn name(&self, mir: &mir::MIR) -> String {
        match self {
            Item::SumConstructorWrapper(sum, var) => mir.variant_names[*sum][*var].clone(),
            Item::Defined(key) => mir.func_names[*key].clone(),
            Item::Autogenerated(_, mfunc) => mfunc.to_string(),
            Item::Method(impl_, method) => {
                mir.func_names[mir.imethods[*impl_][*method].unwrap().inside(impl_.0)].clone()
            }
            Item::Lambda(parent, lkey) => format!("{}:{lkey}", parent.name(mir)),
        }
    }
}
...
pub fn run<'s>(info: ProjectInfo, target: Target, iquery: &ImplIndex, mut mir: mir::MIR) -> Output {
    ...
    let main = {
        let typing = MonoTyping::new(Item::Defined(info.main), Map::new(), returns);
...
impl LIR {
    fn static_func(
        ...
        let typing = morph.apply_typing(Item::Defined(key), &fdef.typing);
...
impl Item {
    pub fn module(&self) -> key::Module {
        match self {
            Item::Autogenerated(module, _)
            | Item::Method(M(module, _), _)
            | Item::SumConstructorWrapper(M(module, _), _)
            | Item::Defined(M(module, _)) => *module,
            Item::Lambda(orig, _) => orig.module(),
        }
    }

    fn get_root_fdef<'m>(&self, mir: &'m mir::MIR) -> &'m mir::Function {
        match self {
            Item::Defined(f) => mir.funcs[*f].as_done(),
            Item::Method(ikey, m) => {
                let f = mir.imethods[*ikey][*m].unwrap();
                mir.funcs[f].as_done()
            }
            Item::Lambda(origin, _) => origin.get_root_fdef(mir),
            _ => {
                panic!("autogenerated functions have no mir function definition")
            }
        }
    }
}
...
impl<'a> FuncLower<'a> {
    ...
    fn expr_of_origin(&mut self, f: Item) -> &'a mir::Expr {
        match f {
            ...
            Item::Method(imp, method) => {
                let fkey = self.mir.imethods[imp][method].unwrap();
                &self.mir.funcs[fkey].as_done().expr
            }
            Item::Lambda(new, lkey) => match *new {
                Item::Defined(func) => &self.mir.funcs[func].as_done().lambdas[lkey].expr,
                Item::Method(imp, method) => {
                    let fkey = self.mir.imethods[imp][method].unwrap();
            ...
            _ => {
                panic!("auto generated functions have no mir function definition")
            }
        }
    }
    ...
    fn monofunc_wrapper_for_sumvar(
        ...
        let origin = Item::SumConstructorWrapper(sum, var);
        ...
        match self.lir.mono_resolve.get(&key) {
            ...
            None => {
                ...
                let kind = Item::SumConstructorWrapper(sum, var);
                let mut function = Function::new(symbol, kind, ssa, ty, 1);
        ...
        match self.lir.stringable {
            ...
            None => {
                ...
                let [split_at, split_while, split_first, equals, from_raw_parts] = [0, 1, 2, 3, 4]
                    .map(key::Method)
                    .map(|method| Item::Method(ikey, method))
    ...
    fn get_lambda_origin(&self, origin: Item, lambda: key::Lambda) -> (Item, &'a [key::Bind]) {
        match &origin {
            Item::Defined(f) => {
                let origin = Item::Lambda(Box::new(origin.clone()), lambda);
            ...
            Item::Lambda(origin, _) => self.get_lambda_origin((**origin).clone(), lambda),
            _ => unreachable!(),
        }
    }
        ...
        match callable {
            mir::Callable::Func(M(module, key), mapper) => match key {
                ...
                ast::NFunc::Method(key, method) => {
                    ...
                    let forigin = Item::Method(ikey, *method);
...
fn func_symbol(mir: &mir::MIR, key: MonoFunc, origin: &Item) -> String {
...
impl fmt::Display for Item {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            Item::SumConstructorWrapper(sum, var) => write!(f, "{sum}:{var}"),
            Item::Autogenerated(module, mfunc) => write!(f, "{module}:{mfunc}"),
            Item::Defined(func) => func.fmt(f),
            Item::Method(ikey, method) => write!(f, "{ikey}:{method}"),
            Item::Lambda(parent, lambda) => write!(f, "{parent}:{lambda}"),
        }
    }
}
